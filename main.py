...
def xm_md5():
    ...
    try:
        ...
        sign = str(hashlib.md5("himalaya-{}".format(html.text).encode()).hexdigest()) + "({})".format(
            str(round(random.random() * 100))) + html.text + "({})".format(str(round(random.random() * 100))) + nowTime
    except:
...
def download():
    ...
    for n in range(0, len(xuanzhong_index)):
        ...
        if not verify_full_url(url):
            Text1.insert(END, '> ' + file_name + '无法下载\n')
            Text1.see(END)
            continue
...
def verify_full_url(target):
    url_pattern = re.compile(r'(http|ftp|https)://\w+\.\w+')
